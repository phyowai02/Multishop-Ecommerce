  ...
  const fetchOrder = async () => {
    try {
      const response = await axios.get(
        `https://multishop-ecommerce-wbac.onrender.com/api/order/${id}`,
        {
          headers: {
            "x-access-token": token,
          },
  ...
  const updateOrderStatus = async (status) => {
    const itemsToUpdate = order.items.map((item) => {
      const updatedQuantity =
        quantities[item.uniqueId] !== undefined
          ? quantities[item.uniqueId]
          : item.quantity;
      return {
        _id: item._id,
    ...
    try {
      ...
      const response = await axios.put(
        `https://multishop-ecommerce-wbac.onrender.com/api/order/${id}/status`,
        {
          items: itemsToUpdate,
          status: status,
        },
      ...
      alert(
        `Order status updated to ${response.data.order.status} successfully!`
      );
      fetchOrder();
    ...
    <div className="h-screen flex">
      ...
      <div className="sideB w-full h-full">
        <div className="flex justify-between p-2 text-slate-600 text-lg mx-3">
          <div>
            <h1 className="cursor-pointer rounded-lg p-2">
          ...
          <div className="flex">
            ...
            <div
              className="flex items-center cursor-pointer rounded-lg text-white p-2 bg-lime-600"
        ...
        <div className="mx-9 my-6 flex justify-between">
          <h1 className="font-semibold text-slate-600">
            Order Details ({order.items ? order.items.length : 0})
          </h1>
            ...
            {order.items.length > 0 ? (
              order.items.map((item) => {
                ...
                return (
                  <div
                    key={item._id}
                    className="flex bg-white p-3 my-2 rounded-lg border"
                  >
                    <div className="w-[38%] grid grid-cols-2">
                      <div className="flex items-center">
                        <img
