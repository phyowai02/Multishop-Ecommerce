...
const Sidebar = () => {
  ...
  return (
    <div>
      ...
      <nav className="flex justify-between items-center px-5 py-3">
        <div className="container block md:hidden w-16 z-50">
          <div className="menuContainer ms-2" onClick={toggleMenu}>
            ...
            <div
              className={`lineThree ${isOpened ? "rotateLineThree" : ""}`}
            ></div>
          </div>
        </div>
      ...
      <div
        className={`flex justify-center items-center font-bold text-lg text-white w-full h-screen absolute top-0 left-0 bg-black bg-opacity-95 z-40 ${
